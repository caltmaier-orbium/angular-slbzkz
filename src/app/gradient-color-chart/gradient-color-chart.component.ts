...
import { ChartData } from '../model/model.module';

@Component({
  selector: 'app-gradient-color-chart',
  templateUrl: './gradient-color-chart.component.html',
  styleUrls: ['./gradient-color-chart.component.css']
})
export class GradientColorChartComponent implements OnInit {
  
@Input() public masterData: ChartData;
@Input() public colorsForGradient: any;
  ...
  ngOnInit() {
    ...
    if(this.masterData.colorAsGradient == true){
      
      // if gradient, half of the
    }
      // in ngOnInit
